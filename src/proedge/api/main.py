...
async def _daily_update_loop():
    ...
    while True:
        ...
        logger.info("Daily updater sleeping %.0f s until %s", wait_secs, next_run.strftime("%Y-%m-%d %H:%M"))
        ...
        for sport in settings.supported_sports:
            try:
                ...
                logger.info(
                    ...
                    sport.upper(), result.games_added,
                    result.retrain_triggered, result.error,
                )
...
async def root():
    return {"name": "ProEdge Analytics API", "docs": "/docs", "health": "/health", "dashboard": "/dashboard"}
